...
**复制下面整段指令，粘贴给任意 AI 助手**（Claude / CodeBuddy / Cursor / DeepSeek 等），它会读取 `AGENTS.md` 并替你完成从链接到 KOReader 可用的全部步骤：

```text
你是「微信读书 → KOReader」下载部署助手。请先读取本仓库根目录的 AGENTS.md 并严格按它执行。
我的目标：把一本微信读书书籍下载到 KOReader 设备使用（正文 + 划线高亮 + 点击划线弹想法 + 进度/时长同步）。
请按以下顺序工作：
1. 读取 AGENTS.md，完成环境检查（Python 3.10+、weread.koplugin 依赖存在、网络直连）；
2. 向我索取必需信息：书的阅读页链接或 bookId、Cookie（或设备上 weread.lua 的文件路径）、API Key；
3. 按 AGENTS.md 第 3 步下载书籍（全量或我指定的章数），若遇限流(-10102)/超时(499)按其中方法处理，断点续传；
4. 按第 4 步生成想法库 thoughts.db；
5. 输出需要拷入设备的文件路径（.epub 与 thoughts.db）和 KOReader 端操作步骤，并完成第 6 步验证清单后向我汇报；
6. 我的凭证不要写入任何日志或提交记录；无法解决的问题先按第 7 步故障速查处理，仍不行就停下来问我。
开始吧。
```

> 说明：`AGENTS.md` 是给 AI 看的完整执行手册（环境 → 凭证 → 下载 → 想法库 → 导入 → 验证 → 故障速查），上面这段是触发它的入口指令。
