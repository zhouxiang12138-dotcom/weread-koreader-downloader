...
把这个仓库丢给任意 AI 助手（Claude / CodeBuddy / Cursor 等），让它读取 **`AGENTS.md`** 并执行即可——从拿到书的链接到 KOReader 可用，全程自动。`AGENTS.md` 包含完整可执行流程：环境检查 → 凭证获取 → 下载（含限流/断点处理）→ 想法库生成 → 设备导入 → 交付验证清单 → 故障速查。
